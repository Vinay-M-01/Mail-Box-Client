...
const OutBoxSingleMail = (props) => {
  ...
  const deleteClickHandler = () => {
    fetch(
      `https://mailbox-client-f1eeb-default-rtdb.firebaseio.com/${cleanUserEmail}/sentemails/${endpoint}.json`,
      {
        method: "DELETE",
        headers: {
      ...
      if (res.ok) {
        fetch(
          `https://mailbox-client-f1eeb-default-rtdb.firebaseio.com/${cleanUserEmail}/sentemails.json`
        )
          .then((res) => res.json())
          .then((data) => {
            props.onDelete(data);
